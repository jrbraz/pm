...
def _get_board(client: TestClient, username: str, board_id: int) -> dict:
    r = client.get(f"/api/users/{username}/boards/{board_id}")
    return r.json()["board"]


def _save_board(client: TestClient, username: str, board_id: int, board: dict) -> None:
    r = client.put(f"/api/users/{username}/boards/{board_id}", json=board)
...
def test_card_with_checklist_round_trips(client: TestClient) -> None:
    ...
    r = client.get("/api/users/alice/board")
    board = r.json()["board"]
    ...
    r = client.put("/api/users/alice/board", json=board)
    assert r.status_code == 200
...
def test_card_checklist_check_off(client: TestClient) -> None:
    ...
    client.put("/api/users/alice/board", json=board)
...
def test_board_stats_includes_checklist_fields(client: TestClient) -> None:
    ...
    r = client.post("/api/users/alice/boards", json={"name": "Stats Board"})
    board_id = r.json()["id"]

    board = client.get(f"/api/users/alice/boards/{board_id}").json()["board"]
    first_card_id = board["columns"][0]["cardIds"][0]
    board["cards"][first_card_id]["checklist"] = [
        {"id": "item-1", "text": "Done item", "done": True},
        {"id": "item-2", "text": "Pending item", "done": False},
    ]
    client.put(f"/api/users/alice/boards/{board_id}", json=board)

    r = client.get(f"/api/users/alice/boards/{board_id}/stats")
    assert r.status_code == 200
    data = r.json()
