...
class TestChatEndpoint:
    ...
    def test_chat_only_response(self, mock_process, auth_client):
        ...
        response = client.post(f"/api/users/{username}/chat", json={"message": "hi"})
    ...
    @patch("app.routes.chat.process_chat")
    def test_chat_with_board_update_saves(self, mock_process, mock_save, auth_client):
        client, token, username = auth_client
        from app.ai_chat import ChatResult
        ...
        response = client.post(f"/api/users/{username}/chat", json={"message": "add card"})
    ...
    def test_empty_message_returns_400(self, auth_client):
        client, token, username = auth_client
        response = client.post(f"/api/users/{username}/chat", json={"message": ""})
        assert response.status_code == 400
        assert response.json()["error"]["code"] == "VALIDATION_ERROR"

    def test_missing_message_returns_422(self, auth_client):
        client, token, username = auth_client
        response = client.post(f"/api/users/{username}/chat", json={})
        assert response.status_code == 422

    @patch("app.routes.chat.process_chat")
    def test_ai_error_returns_502(self, mock_process, auth_client):
        client, token, username = auth_client
    ...
    @patch("app.routes.chat.process_chat")
    def test_passes_history(self, mock_process, auth_client):
        client, token, username = auth_client
        from app.ai_chat import ChatResult
        ...
        client.post(f"/api/users/{username}/chat", json={"message": "next", "history": history})
